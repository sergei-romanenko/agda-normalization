...
eval-embVar≤⇓ : ∀ {α Β Γ} (η : Β ≤ Γ) (x : Var Γ α) →
  eval (embVar x) (env≤ η (id-env Γ)) ⇓ ne (var (var≤ η x))
eval-embVar≤⇓ η zero = now⇓
eval-embVar≤⇓ {α} {Β} {γ ∷ Γ} η (suc x)
  rewrite env≤ η (env≤ wk (id-env Γ)) ≡
    env≤ (η ● wk) (id-env Γ) ∋ env≤∘ η wk (id-env Γ) |
          var≤ η (suc x) ≡ var≤ (η ● wk) x ∋ var≤∘suc η x
  = eval-embVar≤⇓ (η ● wk) x

eval-embVar⇓ : ∀ {α Γ} (x : Var Γ α) →
  eval (embVar x) (id-env Γ) ⇓ ne (var x)
eval-embVar⇓ {α} {Γ} x
  with eval-embVar≤⇓ ≤id x
... | r
  rewrite env≤ ≤id (id-env Γ) ≡
    id-env Γ ∋ env≤-≤id {Γ} {Γ} (id-env Γ) |
          var≤ ≤id x ≡ x ∋ var≤-≤id x
  = r
